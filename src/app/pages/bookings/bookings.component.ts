...
export class BookingsComponent implements OnInit {
  ...
  selecionar_dia() {
    console.log(this.dia_selecionado.day)
  }
  ...
  changeCustomerType(index) {
    ...
    if (numero > 0) {
      ...
      console.log(this.ArrayCustomerTypesSelecionado);
    ...
    for (const iterator of this.ArrayCustomerTypesSelecionado) {
      ...
      if (array_precio_tempo == 0) {
        ...
        Swal.fire({
          ...
          title: 'Error',
          allowEnterKey: false,
          text: `No prices were found for the type of person ${iterator.label} related to category ${this.CategoriesSelecionado.label} and the type  ${this.typeSelecionado.label}`
        }).then((result) => {
  ...
  seachPrecio(customerTypeId) {
    if (
      ...
      console.log("Categoria:" + this.CategoriesSelecionado.value)
      console.log("type:" + this.typeSelecionado.value)
      console.log("customerTypeId:" + customerTypeId)
     // console.table(this.jsonTour.prices);
  ...
  totales_personas() {
    console.log(this.ArrayCustomerTypesSelecionado);
    // console.log(this.jsonTour.hotel);
    let precio = 0;
    for (const iterator of this.ArrayCustomerTypesSelecionado) {
  ...
  changeCantAdditionalServices(p, i) {
    ...
    console.log(this.ArrayAdditionalServices)
    this.totalAdditionalServices();
  }
  ...
  llenarAdditionalServices() {
    ...
    console.log(this.jsonTour.additionalServices)
    this.totalAdditionalServices();
  }
  ...
  saveHotel() {
    ...
    if (
      ...
      console.log(this.hoteles_selecionados);
      console.log(this.jsonTour.hotel);
      // let precio = 0;
      ...
      if (precio_array != undefined) {
        ...
        Swal.fire({
          ...
          title: 'Error',
          allowEnterKey: false,
          text: ` the type of hotel ${tipo.label} with the type of room ${catogoria.label}  is not configured `
        }).then((result) => {
        })
      }
    ...
    for (const iterator of this.ArrayCustomerTypesSelecionado) {
      switch (iterator.value) {
        ...
        case "1347fe4e-8192-eb11-ab1b-40ec994df7b4":                                        // check for Rudy
          ...
          console.log('hola' + studentPassengers)
          break;
  ...
  reservar() {
    if (this.nuevo_usuario != false) {
      ...
      if (
        ...
        //     title: 'Error',
        //     allowEnterKey: false,
        //     text: "Aceptar los Terminos y condiciones es necesario para Registrar"
        //   }).then((result) => {
        //   })
        // }else{
        var data = {
        ...
        this._authenticationservice.postRegister(data).subscribe((res: any) => {
          if (res == true) {
            console.log(res);
            this.login(this.emailNuevo, this.contrasenaNuevo)
          }
  ...
  AsignarPago(tipo) {
    this.montoPagar = tipo == 'T' ? (this.total_AdditionalServices + this.total_hotel + this.total_persona) : ((this.total_hotel + this.total_persona) / 2);
    console.log(this.montoPagar);
  }
  savetour() {
    if (this.terminoNuevo == false) {
      Swal.fire({
      ...
      this._bookingsservice.postBookings(data).subscribe((res: any) => {
        if (res) {
          ...
          // this.
          // this._bookingsservice.postBookings(data).subscribe((res: any) => {
          // /api/tour / password / payme / { amount }
          // })
          ...
          setTimeout(() => {
            if (this.gateWayPayment == true) {
              console.log('btn-prod')
              console.log($('#acquirerId').val())
              console.log($('#idCommerce').val())
              console.log($('#purchaseAmount').val())
              console.log($('#purchaseCurrencyCode').val())
              console.log($('#purchaseOperationNumber').val())
              let element: HTMLElement = document.getElementById('prod') as HTMLElement;
              element.click()
            } else {
              console.log('btn-integra')
